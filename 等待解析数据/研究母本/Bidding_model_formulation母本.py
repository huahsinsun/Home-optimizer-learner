...
# 字体设置
plt.rcParams['font.sans-serif'] = ['Microsoft YaHei']  # 设置中文字体为仿宋GB2312
...
class VPP_function():   # Initialize the VPP environment, formulate the VPP functions
    ...
    def __init__(self,Energy_price_forecast,Load_forecast,RES_forecast,Time_slots): # Environment and parameters initialization
        ...
        self.Bid_segment_number = 4     # 投标分段数（3-10段）
    ...
    def VPP_bid_curve_Quantity_price_two_step_algorithm(self):
        #竞标价格-数量曲线，分成两步进行：第一步，先基于线性规划计算出来竞标期望出清值，然后在出清值附近，对竞标曲线离散化
        # （越靠近出清功率，电价段变化越大，功率段越小）
        ...
        if self.Segment_split_mode == 0:  # 均匀分解模式
            ...
            for time_t in range(self.Time_slots):
                ...
                for seg_i in range(1,int(self.Bid_segment_number/2)):
                    Quantity_curve[seg_i,time_t] = copy.deepcopy(self.VPP_P_overall_expect[time_t]) -\
                    (int(self.Bid_segment_number/2) - seg_i)*(Quantity_curve[int(self.Bid_segment_number/2), time_t] -\
                     self.Quantity_min[time_t]) / int(self.Bid_segment_number / 2)
            ...
            print('*'*50,'展示','*'*50)
            print('Quantity_bid_seg')
            print(self.Quantity_bid_seg.shape)
            print('Price_bid_curve')
            print(self.Price_bid_curve.shape)
            print('*' * 50, '结束', '*' * 50)

            self.Quantity_curve1 =self.Quantity_bid_seg.reshape(24)

            self.Price_curve1 = self.Price_bid_curve.reshape(24)
            plt.plot(range(0,24,4),self.VPP_P_overall_expect)
            plt.show()
            plt.plot(range(0,24,4),self.Energy_price_forecast,marker='s')
            plt.show()

            plt.plot(range(24),self.Quantity_curve1,marker='s',label='报量')
            plt.legend()
            plt.show()
            plt.plot(range(24),self.Price_curve1,marker='d',label='报价')
            plt.legend()
            plt.show()
            ...
            fig2 = plt.figure(2, figsize=(10, 8))  # Adjust figure size as needed
            for time_t in range(self.Time_slots):
                ax2 = fig2.add_subplot(int((self.Time_slots + 1) // 2), 2, time_t + 1)
                ax2.step(Quantity_curve[:, time_t], Price_curve[:, time_t])
                ax2.set_xlim([self.Quantity_min[time_t], self.Quantity_max[time_t]])
                ax2.set_ylim([np.min(Price_curve), np.max(Price_curve)])
                ax2.set_title(f'Quantity_price_curve time {time_t + 1}')
    ...
    def VPP_bidding_Price_Quantity_mode(self):
        ...
        self.Quantity_curve1=Quantity_curve.reshape(24)
        self.Price_curve1=Price_curve.reshape(24)

        # 直接将Quantity_clear_seg转换为numpy数组，不使用.X
        Quantity_clear_seg_array = np.array(
            [self.Quantity_clear_seg[seg, t] for seg in range(self.Bid_segment_number) for t in range(self.Time_slots)])
        self.clear_q = Quantity_clear_seg_array.reshape(24)
        fig, ax = plt.subplots(figsize=(12, 8))
        fig.tight_layout(pad=4.0)
        x_label = np.arange(24)  # 0到23的整数，表示24小时
        # 主Y轴：数量
        ax.step(x_label, self.Quantity_curve1, where='post', label='投标数量', color='b')
        ax.step(x_label, self.clear_q, where='post', label='出清数量', color='g', linestyle='--')
        ax.set_xlabel('一天中的小时')
        ax.set_ylabel('数量', color='b')
        ax.tick_params(axis='y', labelcolor='b')
        # 副Y轴：价格
        ax2 = ax.twinx()
        ax2.step(x_label, self.Price_curve1, where='post', label='价格', linestyle=':', color='r')
        ax2.set_ylabel('价格', color='r')
    ...
    def Result_plot(self):
        ...
        for time_t in range(self.Time_slots):
            Energy_price_real[time_t] = self.Energy_price_forecast[time_t] * (
                        1 + price_uncertain_ratio * np.random.randn())
        for time_t in range(self.Time_slots):
            for Seg_i in range(self.Bid_segment_number):
            ...
            Market_clearing_quantity_curve_real[time_t] = np.sum(Market_clearing_quantity_curve_segment[:, time_t]) + \
        ...
        fig, ax = plt.subplots(figsize=(12, 8))
        fig.tight_layout(pad=4.0)
        x_label = np.arange(0,24,4)  # 假设Time_slots是24
        # 主Y轴：数量
        ax.step(x_label, self.Market_clearing_quantity_curve_real, where='post', label='实际出清数量', color='g',
                linestyle='--')
        # 计算总投标数量曲线
        total_bid_quantity = np.sum(self.Quantity_bid_seg, axis=0) + self.Quantity_min
        ax.step(x_label, total_bid_quantity, where='post', label='总投标数量', color='b')
        ax.set_xlabel('一天中的小时')
        ax.set_ylabel('数量 (MWh)', color='b')
        ax.set_title('市场出清模拟')
        ax.tick_params(axis='y', labelcolor='b')
        # 副Y轴：价格
        ax2 = ax.twinx()
        ax2.step(x_label, self.Energy_price_real, where='post', label='实际价格', linestyle=':', color='r')
        ax2.set_ylabel('价格 ($/MWh)', color='r')
        ax2.tick_params(axis='y', labelcolor='r')
        lines, labels = ax.get_legend_handles_labels()
        lines2, labels2 = ax2.get_legend_handles_labels()
        ax.legend(lines + lines2, labels + labels2, loc='upper left')
        ...
        # plt.axis([0, self.Time_slots, 0.5 * min(self.Energy_price_forecast[0:self.Time_slots]),
        #           1.5 * max(self.Energy_price_forecast[0:self.Time_slots])])
        # ax1.plot(x_label_1, self.Energy_price_forecast[0:self.Time_slots], label='预测电价')
        # ax1.plot(x_label_1, self.Energy_price_real, linestyle=':', label='实际电价')
        # plt.title('预测电价和实际电价')
        # plt.legend()
        #
        # ax1 = fig1.add_subplot(3, 2, 2)
        # plt.axis([0, self.Time_slots, -1.0, 1.0])
        # ax1.plot(x_label_1, self.RES_forecast[0:self.Time_slots], color='r', label='可再生能源预测')
        # ax1.plot(x_label_1, self.Load_forecast[0:self.Time_slots], label='负荷需求')
        # ax1.plot(x_label_1, self.RES_P, color='g', linestyle=':', label='可再生能源发电量')
        # plt.title('可再生能源预测、负荷需求和可再生能源发电量')
        # plt.legend()
        #
        # ax1 = fig1.add_subplot(3, 2, 3)
        # plt.axis([0, self.Time_slots, min(self.VPP_P_overall_expect), max(self.VPP_P_overall_expect)])
        # ax1.plot(x_label_1, self.VPP_P_overall_expect, color='r', label='期望VPP')
        # ax1.plot(x_label_1, self.VPP_P_overall_real, color='g', label='实际VPP')
        # ax1.plot(x_label_1, self.Market_clearing_quantity_curve_real, color='b', linestyle=':', label='实际出清曲线')
        # plt.title('期望VPP和实际出清曲线')
        # plt.legend()
        #
        # ax1 = fig1.add_subplot(3, 2, 4)
        # plt.axis([0, self.Time_slots, 0, max(self.ESS_energy_capacity) * 1.1])
        # for ESS_i in range(self.soc_num):
        #     ax1.plot(x_label_1, np.dot(self.ESS_energy_capacity[ESS_i], np.ones(self.Time_slots)), linestyle=':',
        #              label=f'ESS{ESS_i + 1}容量')
        #     ax1.plot(x_label_1, self.ESS_energy_var[ESS_i, :], label=f'ESS{ESS_i + 1}能量变化')
        # plt.title('储能系统能量变化')
        # plt.legend()
        #
        ...
        # for ESS_i in range(self.soc_num):
        #     ax1.plot(x_label_1, self.ESS_Pin_var[ESS_i, :], label=f'ESS{ESS_i + 1}充电功率')
        #     ax1.plot(x_label_1, -self.ESS_Pout_var[ESS_i, :], linestyle=':', label=f'ESS{ESS_i + 1}放电功率')
        # plt.title('储能系统充放电功率')
        # plt.legend()
        #
        ...
        # fig2 = plt.figure(2)
        # for time_t in range(self.Time_slots):
        #     ax2 = fig2.add_subplot(int((self.Time_slots / 2)), 2, time_t + 1)
        #     plt.axis([self.Quantity_min[time_t], self.Quantity_max[time_t], np.min(Price_curve), np.max(Price_curve)])
        #     ax2.step(Quantity_curve[:, time_t], Price_curve[:, time_t])
        #     plt.title('数量-价格曲线')
        #
        # plt.show()

        # return Quantity_curve, Price_curve
...
def Initialization_data():  # load the initialization simulation_cap_data, including the price, load, renewable energy, overall time slots, etc.
    ...
    # 预测电价 = [16.07,14.96,14.28,13.26,13.21,14.08,15,16.93,18.6,20.41,22.11,24.5,26.57,28.07,29.97,30.66,35.5,35.58,29.54,26.73,23.91,22.16,19.93,17.93]
    ...
    # 可再生能源预测 = [0,0,0,0,0,0,0.0341275572,0.2688327316,0.4466907341,0.7883032491,0.803898917,1.0,0.9681829122,0.8001925391,0.659253911,0.34811071,0.2309025271,0.0895788207,0.0074127557,0,0,0,0,0]
...
args.mode = "Price-Quantity mode (two-step)"
...
# 执行选定的投标策略
if args.mode == "Quantity only mode":
    #没有后续结果,单一程序报表
    VPP_env.VPP_bidding_Quantity_only_mode()
elif args.mode == "Price-Quantity mode (one-step)":
    VPP_env.VPP_bidding_Price_Quantity_mode()
elif args.mode == "Price-Quantity mode (two-step)":
    VPP_env.VPP_bid_curve_Quantity_price_two_step_algorithm()
    VPP_env.Result_plot()
else:
    print("Invalid mode selected. Please choose 'Quantity only mode', 'Price-Quantity mode (one-step)', or 'Price-Quantity mode (two-step)'.")
